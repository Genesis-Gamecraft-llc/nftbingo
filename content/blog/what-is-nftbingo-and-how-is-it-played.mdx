...
When a number is called, players check their cards to see if that number appears on their card.

As numbers are drawn, players look for a winning pattern, such as:
...
When a card completes the winning pattern, the player calls “BINGO” and the game is resolved based on the number that completed the pattern.
...
In NFTBingo, each bingo card is an NFT. This NFT contains embedded metadata that permanently defines the card.
