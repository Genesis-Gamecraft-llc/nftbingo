...
export default function MintPage() {
  ...
  const RPC_RAW = process.env.NEXT_PUBLIC_SOLANA_RPC_URL || "";
  const RPC =
    typeof window !== "undefined" && RPC_RAW.startsWith("/")
      ? new URL(RPC_RAW, window.location.origin).toString()
      : RPC_RAW;

  const IRYS_URL = process.env.NEXT_PUBLIC_IRYS_URL || "https://node1.irys.xyz";
  ...
  async function doPlayerMintOne() {
    ...
    if (!RPC) return setPlayerErr("Missing NEXT_PUBLIC_SOLANA_RPC_URL.");
    ...
    try {
      ...
      const initRes = await fetch("/api/player-mint/init", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ wallet: pubkey, count: 1 }), // ✅ locked
      });
      ...
      // upload image
      const imageFile = createGenericFile(base64ToBytes(pkg.pngBase64), `player-${pkg.serialStr}.png`, {
        contentType: "image/png",
      });
      const [imageUri] = await umi.uploader.upload([imageFile]);

      // upload metadata
      const md = structuredClone(pkg.metadataTemplate);
      md.image = imageUri;
      if (md.properties?.files?.[0]) md.properties.files[0].uri = imageUri;

      const mdBytes = new TextEncoder().encode(JSON.stringify(md));
      const mdFile = createGenericFile(mdBytes, `player-${pkg.serialStr}.json`, { contentType: "application/json" });
      ...
      const buildRes = await fetch("/api/player-mint/build", {
        ...
        body: JSON.stringify({
          ...
          wallet: pubkey,
          items: [{ index: pkg.index, imageUri, metadataUri }],
        }),
      });
      ...
      if (!txBase64) throw new Error("build did not return txBase64");
      ...
      const submitRes = await fetch("/api/player-mint/submit", {
        ...
        body: JSON.stringify({
          ...
          signedTxBase64,           // for single-submit routes
          signedTxBase64s: [signedTxBase64], // for array-submit routes
        }),
      });
      ...
      setPlayerLastMint({ imageUri, metadataUri, signature: sig });
    } catch (e: any) {
